    ...
    pub fn from_file(pth:&PathBuf) -> Result<HtAccess> {
        ...
        if pth.exists() {
            read_to_string(pth)?
                ...
                .for_each(|line| {
                    match Credential::from_line(line) {
                        None => {}
                        Some(cred) => {c.insert(cred.name.clone(), cred);}
                    }
                })
        }
        return Ok(HtAccess{path: pth.clone(), credentials:c});
    }
...
impl Credential {
    ...
    pub fn to_line(&self) -> String {
        if self.hash_val.is_some() {
            return format!("{}:{}\n", self.name.as_str(), self.hash_val.as_ref().unwrap())
        } else {
            "".into()
        }
    }
